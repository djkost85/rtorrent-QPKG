<?php
  ...
  * along with this program.  If not, see <http://www.gnu.org/licenses/>.
		*
	 */
$GLOBALS['uploadHack']=false; //Use xmlrpc command line ot get uploaded bytes
...
class Rtorrent
{
	...
	public function Retrieve($id='')
	{
		...
		$message = new xmlrpcmsg('d.multicall', array(
			new xmlrpcval('name'), //Make an array for all loaded torrents - 0
			new xmlrpcval('d.get_hash='), //The torrent hash 1
			new xmlrpcval('d.get_name='), //Torrent's name 2
			new xmlrpcval('d.get_state='), //0 = stopped, 1 = started 3
			new xmlrpcval('d.get_size_bytes='), //The size in bytes 4
			new xmlrpcval('d.get_bytes_done='), //How many bytes completed 5
			new xmlrpcval('d.get_up_total='), //How much in total has been uploaded 6
			new xmlrpcval('d.get_down_rate='), //Download rate in bytes 7
			new xmlrpcval('d.get_up_rate='), //Upload rate in bytes 8
			new xmlrpcval('d.get_peers_connected='), //Amount of connected peers 9
			new xmlrpcval('d.get_peers_not_connected='), //Amount of unconnected peers 10
			new xmlrpcval('d.get_peers_accounted='), //Number of leechers 11
			new xmlrpcval('d.get_complete='), //Is the torrent completely downloaded? 12
			new xmlrpcval('d.is_hash_checking='), //Is it rehashing? 13
			new xmlrpcval('d.get_creation_date='), //Date torrent added 14
			new xmlrpcval('d.get_base_path='), //Where the torrent exists 15
			new xmlrpcval('d.get_free_diskspace='), //Free disk space where torrent is 16
			new xmlrpcval('d.is_private='), //Is torrent private? 17
			new xmlrpcval('d.get_message='), //Comment 18
			new xmlrpcval('d.get_priority='), //Priority (number) 19
			new xmlrpcval('d.is_hash_checked='), //Has it been hash checked before? 20
			new xmlrpcval('d.get_skip_total='), //How many wasted bytes? 21
			new xmlrpcval('d.get_custom5='), //We use this for the torrents "new name" 22
			new xmlrpcval('d.get_custom4='), //We use this for the torrents "label" 23
			...
			new xmlrpcval('d.get_chunk_size='), //Get the size of a single chunk in bytes 24
			new xmlrpcval('d.get_size_chunks='), //Get how many chunks are in the torrent 25
			new xmlrpcval('d.get_completed_chunks=') //Get how many chunks have downloaded. 26
		) );
		...
		foreach($torrents as $torrent)
		{
				...
				$uploadedSize = $torrent[5];
				if($GLOBALS['uploadHack']==true) {
					unset($strA);
					exec("xmlrpc localhost d.get_up_total ".$torrent[0],$strA);
					$str = substr($strA[2],16);
					$uploadedSize = $str;
				}
	...
	{
		$message = new xmlrpcmsg('get_max_memory_usage');
		return $this->SendAndRecieve($message);
	}
	public function GetMemoryUsage()
	{
		$message = new xmlrpcmsg('get_memory_usage');
		return $this->SendAndRecieve($message);
	}
	
	public function GetDownThrottle()
	{
		$message = new xmlrpcmsg('throttle.global_down.max_rate');
		return $this->SendAndRecieve($message);
	}
	public function GetUpThrottle()
	{
		$message = new xmlrpcmsg('throttle.global_up.max_rate');
		return $this->SendAndRecieve($message);
	}
	
	public function GetDownRate()
	{
		$message = new xmlrpcmsg('get_down_rate');
		return $this->SendAndRecieve($message);
	}
	public function GetUpRate()
	{
		$message = new xmlrpcmsg('get_up_rate');
		return $this->SendAndRecieve($message);
	}
